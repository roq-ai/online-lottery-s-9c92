...
import { useRoqPlatformUserProfiles, useLotteryGameCount, useWinnerCount, useTeamMemberCount } from 'lib/roq';

import { WinnerListPage as ShowCaseList } from 'pages/winners';

function DashboardPage() {
  const { session } = useSession();

  const lotteryGameStat = useLotteryGameCount();

  const winnerStat = useWinnerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Lottery Games',
      result: lotteryGameStat?.data,
      isLoading: lotteryGameStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Winners',
      result: winnerStat?.data,
      isLoading: winnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Team Members',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`winners`} passHref legacyBehavior>
                  <Button
