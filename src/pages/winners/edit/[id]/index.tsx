...
import { useRoqClient, useWinnerFindFirst } from 'lib/roq';
...
function WinnerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useWinnerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: WinnerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.winner.update({
        data: values as RoqTypes.winner,
        where: {
          id,
        },
      });
